...
// --- START OF FILE game_logic.js ---
// --- DEEL 7      van 8 dit code blok    --- (<<< REVISE: Stop Grid Sound for CS, Capture Sound Timing >>>)
    ...
    try {
        ...
        for (let i = enemies.length - 1; i >= 0; i--) {
            ...
            const enemyId = enemy.id;
             if (enemy.capturePrepareTimeout && enemy.state !== 'preparing_capture') { /* ... clear capture timeout ... */ clearTimeout(enemy.capturePrepareTimeout); const timeoutIndex = enemySpawnTimeouts.indexOf(enemy.capturePrepareTimeout); if (timeoutIndex > -1) enemySpawnTimeouts.splice(timeoutIndex, 1); enemy.capturePrepareTimeout = null; }
            ...
            switch (enemy.state) {
                 case 'following_bezier_path': {
                    ...
                    // <<< GEWIJZIGD: Off-screen check blijft minder streng voor eerste segment >>>
                 ...
                 case 'following_entrance_path': { /* ... Entrance path logic ... */ /* Debug logs verwijderd */ if (isChallengingStage) break; const pathSegmentsNormal = normalWaveEntrancePaths[enemy.entrancePathId]; if (!pathSegmentsNormal || pathSegmentsNormal.length === 0) { console.error(`Enemy ${enemyId} following invalid path ${enemy.entrancePathId}! Moving to grid.`); enemy.state = 'moving_to_grid'; break; } if (enemy.pathSegmentIndex >= pathSegmentsNormal.length) { enemy.state = 'moving_to_grid'; break; } const currentSegmentNormal = pathSegmentsNormal[enemy.pathSegmentIndex]; if (!currentSegmentNormal?.p0) { console.error(`Enemy ${enemyId} has invalid segment ${enemy.pathSegmentIndex}! Moving to grid.`); enemy.state = 'moving_to_grid'; break; } let pathSpeedMultiplier = 1.0; if (enemy.entrancePathId === 'boss_loop_left' || enemy.entrancePathId === 'boss_loop_right') { pathSpeedMultiplier = BOSS_LOOP_ENTRANCE_PATH_SPEED / NORMAL_ENTRANCE_PATH_SPEED; } const tIncrementNormal = NORMAL_ENTRANCE_PATH_SPEED * pathSpeedMultiplier; enemy.pathT += tIncrementNormal; let pathXNormal, pathYNormal; const oldXNormal = enemy.x; const oldYNormal = enemy.y; if (enemy.pathT >= 1.0) { pathXNormal = calculateBezierPoint(1.0, currentSegmentNormal.p0.x, currentSegmentNormal.p1.x, currentSegmentNormal.p2.x, currentSegmentNormal.p3.x); pathYNormal = calculateBezierPoint(1.0, currentSegmentNormal.p0.y, currentSegmentNormal.p1.y, currentSegmentNormal.p2.y, currentSegmentNormal.p3.y); enemy.pathT = 0; enemy.pathSegmentIndex++; if (enemy.pathSegmentIndex >= pathSegmentsNormal.length) { enemy.state = 'moving_to_grid'; enemy.x = pathXNormal; enemy.y = pathYNormal; } } else { pathXNormal = calculateBezierPoint(enemy.pathT, currentSegmentNormal.p0.x, currentSegmentNormal.p1.x, currentSegmentNormal.p2.x, currentSegmentNormal.p3.x); pathYNormal = calculateBezierPoint(enemy.pathT, currentSegmentNormal.p0.y, currentSegmentNormal.p1.y, currentSegmentNormal.p2.y, currentSegmentNormal.p3.y); } if (enemy.state === 'following_entrance_path') { enemy.velocityX = pathXNormal - oldXNormal; enemy.velocityY = pathYNormal - oldYNormal; enemy.x = pathXNormal; enemy.y = pathYNormal; } break; }
                 ...
                 case 'capturing': {
                    ...
                    if (isShipCaptured && enemy.id !== capturedBossIdWithMessage) {
                        captureBeamActive = false; if(capturingBossId === enemy.id) capturingBossId = null;
                        enemy.state = 'returning';
                        stopSound(captureSound); // Stop sound if state changes due to other capture
                    ...
                    const elapsedCaptureTime = now - enemy.captureStartTime;
                    const halfAnimationTime = CAPTURE_BEAM_ANIMATION_DURATION_MS / 2;
                    const totalBeamStayTime = CAPTURE_BEAM_DURATION_MS;

                    if (elapsedCaptureTime < halfAnimationTime) { captureBeamProgress = elapsedCaptureTime / halfAnimationTime; }
                    else if (elapsedCaptureTime < totalBeamStayTime - halfAnimationTime) { captureBeamProgress = 1.0; }
                    else if (elapsedCaptureTime < totalBeamStayTime) { captureBeamProgress = 1.0 - ((elapsedCaptureTime - (totalBeamStayTime - halfAnimationTime)) / halfAnimationTime); }
                    else { // Beam duration ended
                        captureBeamProgress = 0; captureBeamActive = false; capturingBossId = null;
                        stopSound(captureSound); // Stop sound when beam animation ends
                        enemy.state = 'attacking';
                        enemy.attackPathSegments = generateAttackPath(enemy);
                        enemy.attackPathSegmentIndex = 0; enemy.attackPathT = 0;
                        enemy.speed = BASE_ENEMY_ATTACK_SPEED * ENEMY3_ATTACK_SPEED_FACTOR;
                        enemy.lastFiredTime = 0; enemy.canFireThisDive = true;
                        enemy.attackFormationOffsetX = 0; enemy.attackGroupId = null;
                        break;
                    }
                    captureBeamProgress = Math.max(0, Math.min(1, captureBeamProgress));
